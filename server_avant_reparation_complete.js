...
const getUser=id=>db.prepare("SELECT * FROM users WHERE id=?").get(id),pub=u=>({id:u.id,name:u.name,email:u.email,role:u.role,bio:u.bio||"",avatar:u.avatar||null});
...
app.post("/api/articles/:id/reactions",auth,(req,res)=>{let r=String(req.body.reaction||"");if(!["ðŸ‘","â¤ï¸","ðŸ˜‚","ðŸ˜®","ðŸ˜¡"].includes(r))return res.status(400).json({error:"RÃ©action invalide."});try{db.prepare("INSERT INTO reactions(article_id,reaction,visitor) VALUES(?,?,?)").run(req.params.id,r,String(req.session.userId))}catch(e){}res.json({ok:true})});
app.get("/api/jobs",(req,res)=>res.json(db.prepare("SELECT * FROM jobs WHERE active=1 ORDER BY datetime(created_at) DESC").all()));
app.get("/api/admin/jobs",staff,(req,res)=>res.json(db.prepare("SELECT * FROM jobs ORDER BY active DESC,datetime(created_at) DESC").all()));

app.post("/api/admin/jobs",staff,(req,res)=>{
let{title,company,type,description,contact,image,discord,active}=req.body;
...
let r=db.prepare(`
INSERT INTO jobs
(title,company,type,description,contact,image,discord,active)
VALUES(?,?,?,?,?,?,?,?)
`).run(
title,
company,
...
let r=db.prepare("INSERT INTO jobs(title,company,type,description,contact,image,active) VALUES(?,?,?,?,?,?,?)").run(title,company,type,description,contact||"",image||null,active===false?0:1);
...
let r=db.prepare("UPDATE jobs SET title=?,company=?,type=?,description=?,contact=?,image=?,active=? WHERE id=?").run(title,company,type,description,contact||"",image||null,active?1:0,req.params.id);
...
app.post("/api/admin/citizen-posts/:id/review",staff,async(req,res)=>{let p=db.prepare("SELECT p.*,u.name author FROM citizen_posts p JOIN users u ON u.id=p.user_id WHERE p.id=?").get(req.params.id);if(!p)return res.status(404).json({error:"Annonce introuvable."});let status=req.body.status==="approved"?"approved":"rejected";db.prepare("UPDATE citizen_posts SET status=?,review_note=?,reviewed_at=CURRENT_TIMESTAMP WHERE id=?").run(status,req.body.note||"",p.id);if(status==="approved"){let r=db.prepare("INSERT INTO articles(title,content,excerpt,category,image,author_id,published) VALUES(?,?,?,?,?,?,1)").run(p.title,p.content,p.content.slice(0,220),p.category,p.image,req.session.userId);let a=art(r.lastInsertRowid);let d=await notifyArticle(a);db.prepare("UPDATE articles SET discord_sent=?,discord_error=? WHERE id=?").run(d.ok?1:0,d.ok?null:d.error,a.id)}res.json({ok:true,status})});
...
app.post("/api/admin/gallery",staff,(req,res)=>{let{title,image,caption}=req.body;if(!title||!image)return res.status(400).json({error:"Titre et image requis."});let r=db.prepare("INSERT INTO gallery(title,image,caption,author_id) VALUES(?,?,?,?)").run(title,image,caption||"",req.session.userId);res.json({id:r.lastInsertRowid})});
app.delete("/api/admin/gallery/:id",staff,(req,res)=>{db.prepare("DELETE FROM gallery WHERE id=?").run(req.params.id);res.json({ok:true})});
