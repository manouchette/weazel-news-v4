...
galleryForm.onsubmit = async event => {
    ...
    try {
        ...
        const caption = $("gc").value.trim();
        const youtube = $("gyoutube").value.trim();

        let imageUrl = "";

        if (gi.files && gi.files.length > 0) {
        ...
        if (!imageUrl && !youtube) {
            alert("Ajoutez une photo ou un lien YouTube.");
            return;
        }
    ...
    videoForm.onsubmit = async event => {
        ...
        try {
            ...
            if (!title || !youtube) {
                ...
                alert(
                    "Le titre et le lien YouTube sont obligatoires."
                );
            ...
            alert(
                "Reportage vidéo ajouté avec succès."
            );

        } catch (error) {

            alert(
                "Erreur reportage vidéo : " +
                error.message
            );
        }
    };
    ...
    try {
        ...
        if (!data.length) {
            ...
            videos.innerHTML = `
                <div class="row">
                    <div>
                        <b>Aucun reportage vidéo</b>
                        <small>
                            Les reportages YouTube apparaîtront ici.
                        </small>
                    </div>
                </div>
            `;
...
async function delVideo(id) {
    ...
    if (!confirm("Supprimer ce reportage vidéo ?")) {
        return;
    }
